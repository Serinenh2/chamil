...
export default function PurchaseOrdersPage() {
  ...
  const saveOrder = useSave('purchase-orders')
  const removeOrder = useRemove('purchase-orders')
  ...
  const openEdit = async (row) => {
    ...
    try {
      const detail = await rest.get('purchase-orders', row.id)
      saveOrder.reset()
      setForm({
        id: detail.id,
  ...
  const handleDelete = (row) => {
    if (window.confirm(`${t('purchaseOrders.confirmDelete')} (${row.number})`)) {
      removeOrder.mutate(row.id)
    }
  }
  ...
  const orderTotal = form ? form.line_items.reduce((sum, l) => sum + lineTotal(l), 0) : 0
  ...
  return (
    <>
      ...
      />
      {removeOrder.isError && (
        <Alert tone="error">{errorText(removeOrder.error, t('purchaseOrders.deleteError'))}</Alert>
      )}
      <DataTable columns={columns} rows={data?.results} loading={isLoading} error={error} />

      {form && (
        <Modal title={form.id ? t('purchaseOrders.edit') : t('purchaseOrders.new')}
               onClose={() => setForm(null)} size="xl">
          <form onSubmit={submit}>
            {saveOrder.isError && (
              <Alert tone="error">{errorText(saveOrder.error, t('common.error'))}</Alert>
            )}
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label={t('purchaseOrders.paymentTerm')}>
              ...
              <Field label={t('purchaseOrders.responsible')}>
            ...
            <div className="mt-2 flex items-center justify-between">
              ...
              <span className="text-sm font-semibold">
                {t('table.amount')} : <span className="data">{money(orderTotal)}</span>
              </span>
            </div>
